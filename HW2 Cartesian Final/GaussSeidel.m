function [u,res,iter]= GaussSeidel(Nx,Ny,xmin,xmax,ymin,ymax,itgmr,omega,tol)
            ...
            for i=3:Nx
                for j=3:Ny
                    ...
                    du(L)=(-1/(2*(dx^2+dy^2)/((dx^2)*(dy^2))))*(-rhs(L)-...
                        (1/dx^2*u(ie)+1/dx^2*u(iw)+ ...
                        1/dy^2*u(in)+1/dy^2*u(is)))-u(L);
                    u(L) = omega*(u(L)+du(L)) + (1-omega)*u(L);
            ...
            for i=2:Nx+1
                for j=2:Ny+1
                    ...
                    if i==2 | i==Nx+1 | j==2 | j==Ny+1
                        ...
                        qrr(L) = -(1/dx^2*u(ie)+1/dx^2*u(iw)+1/dy^2*u(in)+1/dy^2*u(is)  ... 
